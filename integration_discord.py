...
commandes={"intro":(["intro"],"Wordpacks c'est quoi ?"),
           "help":(["help"],"afficher la page d'aide aux commandes.",["help <commande>"],"afficher la description d'une commande."),
           "wordpacksprefix":(["wordpacksprefix [prefix]"],"changer le prefix des commandes du bot WordPacks (`!` par défaut). Cette commande fonctionnera toujours avec `!wordpacksprefix` afin de pouvoir corriger d'éventuels changements accidentels. La taille maximale d'un prefix est de 5 caractères."),
           "inscription":(["kukujariv","inscription"],"s'inscrire en tant que dresseur."),
           "info":(["info"],"afficher diverses informations sur votre profil de dresseur.",["info <dresseur>"],"afficher la carte d'infos d'un autre dresseur."),
           "classement":(["classement"],"afficher le classement des 10 meilleurs dresseurs, ainsi que votre position actuelle."),
           "quitter":(["quitter"],"supprimer votre profil de dresseur ainsi que tous vos mots, **définitivement**. Une confirmation vous sera demandée."),
           "mokedex":(["mokedex"],"afficher les mots que vous possédez, ainsi que leur rareté. Les mots sont triés par rareté.",["mokedex <dresseur>"],"afficher le mokédex d'un autre dresseur."),
           "peutonpaké":(["existe [mot]","peutonpaké [mot]"],"vérifier si un mot est obtenable dans un booster."),
           "recherche":(["recherche [mot]"],"vérifier si vous possédez un mot.",["recherche [mot] <dresseur>"],"effectuer une recherche dans le mokédex d'un autre dresseur."),
           "booster":(["booster"],"ouvrir un booster de 3 mots ! Vous obtenez 3 boosters toutes les 12 heures."),
           "megabooster":(["megabooster"],"ouvrir 3 boosters de 3 mots, pour 9 mots **SANS DOUBLONS** !"),
           "upgrade":(["upgrade"],"sacrifier 1 booster pour augmenter la rareté de 2 mots aléatoires de votre mokédex.",["upgrade <nombre>"],"sacrifier un certain nombre de boosters pour augmenter la rareté de 2 mots aléatoires de votre mokédex par booster sacrifié."),
           "echange":(["echange [dresseur]"],"proposer un échange avec un dresseur."),
           "echangetoggle":(["echangetoggle"],"activer/désactiver la possibilité pour les dresseurs de vous proposer des échanges."),
           "accepter":(["accepter [dresseur]"],"accepter la proposition d'échange d'un dresseur et créer un channel d'échange temporaire."),
           "acceptertout":(["acceptertout"],"accepter **toutes** les propositions d'échange et créer un channel d'échange temporaire pour chacune d'entre elles."),
           "refuser":(["refuser [dresseur]"],"refuser la proposition d'échange d'un dresseur."),
           "refusertout":(["refusertout"],"refuser **toutes** les propositions d'échange que l'on vous a fait."),
           "annuler":(["annuler [dresseur]"],"(en dehors d'un channel d'échange) annuler la proposition d'échange faite à un dresseur.",["annuler"],"(dans un channel d'échange) annuler l'échange en cours et supprimer le channel temporaire."),
           "annulertout":(["annulertout"],"annuler toutes les propositions d'échange que vous avez faites. Ceci n'annulera pas d'éventuels échanges en cours."),
           "confirmer":(["confirmer"],"(dans un channel d'échange) compléter l'échange."),
           "listemots":(["listemots"],"afficher la liste des mots obtenables dans un booster.")}
...
async def on_message(message):
    if message.author.id == iencli.user.id:  #empêcher que le bot ne détecte ses propres messages
        ...
        await message.reply(f"<a:mokeball:958666482894643200> _**WORDPACKS**_ <a:mokeball:958666482894643200>\n\n\
...
_Légende_ : `{main.get_prefix(message.guild.id)}commande [élement nécessaire] <élement facultatif>` **-** description de la commande")
      ...
      if message.mentions:
        ...
        try:
          await message.reply(main.changer_prefix(message.guild.id,message.content.split(" ")[1]))
        except IndexError:
        ...
        if message.content.startswith(f"{main.get_prefix(message.guild.id)}accepter ") and not main.check_channels_echanges(message.guild.id,message.channel.id) and message.mentions: #accepter la demande en mentionnant qqn
          ...
          await message.reply(main.echangetoggle(message.author.id,message.guild.id))
          ...
          if not message.mentions:
            ...
            await message.reply(f"Dresseur `{message.mentions[0].name}` introuvable.")
            ...
            if main.boosters_dispo(message.author.id,message.guild.id, 3)[0]:
                ...
                await message.reply(f"""Bravo <@{message.author.id}> ! Tu obtiens les mots suivants : **{(', '.join(record))}**{f" et upgrade les mots suivants : **{(', ').join(mots_upgrade)}**" if mots_upgrade else ""}. Il te reste {boosters_restants} boosters !""")
            else:
                if main.cooldown_ready(message.author.id,message.guild.id)[0]:
                    main.remplir_boosters(message.author.id,message.guild.id)
                    record = []
            ...
            if mot:
                await message.reply(f":white_check_mark: Le dresseur `{dresseur.name}` **possède** le mot `{mot[0]}`. Sa rareté est de **{mot[1]}** ({mot[1]-mot[2]}/{mot[1]}).")
            else:
                await message.reply(f":no_entry_sign: Le dresseur `{dresseur.name}` **ne possède pas** le mot '{message.content.split(' ',2)[1]}'.")
  ...
  <@{message.mentions[0].id}>, utilisez les commandes `{main.get_prefix(message.guild.id)}accepter @{message.author.name}` ou `{main.get_prefix(message.guild.id)}refuser @{message.author.name}`. <@{message.author.id}>, utilisez la commande `{main.get_prefix(message.guild.id)}annuler @{message.mentions[0].name}` à tout moment pour annuler cette proposition d'échange, ou `{main.get_prefix(message.guild.id)}annulertout` pour annuler toutes vos propositions d'échange en cours."
            ...
            await message.reply(f"Dresseur '{message.content.split(' ')[1]}' introuvable.")
          ...
          if main.mot_propose(message.author.id,message.channel.id):
            ...
            await message.reply("Veuillez proposer un mot avant de confirmer.")
            ...
            if not main.check_mot(message.content, message.author.id,message.guild.id):
                await message.reply(f"Le dresseur <@{message.author.id}> ne possède pas le mot '{message.content.lower()}'. Veuillez rééssayer.")
            else:
                
                await message.reply(f"Le dresseur <@{message.author.id}> propose le mot '{message.content.lower()}' (rareté : `{main.changer_mot(message.channel.id,message.author.id,message.guild.id,message.content)}`) pour l'échange !")

        elif message.content == f"{main.get_prefix(message.guild.id)}quitter":
            await message.reply(f"Entrez `{main.get_prefix(message.guild.id)}confirmersuppression` afin de confirmer la suppression de votre profil de dresseur.")
          ...
          await message.reply(main.check_mot_existe(message.content.split(" ")[1]))
        
        elif message.content == (f"{main.get_prefix(message.guild.id)}confirmersuppression"):
          await message.reply(main.suppression_dresseur(message.author.id,message.guild.id))

        elif message.content.startswith(f"{main.get_prefix(message.guild.id)}upgrade"):
            try:
                await message.reply(main.upgrade(message.author.id,message.guild.id,int(message.content.split(" ")[1])))
            except:
                await message.reply(main.upgrade(message.author.id,message.guild.id))
        elif message.content.startswith(f"{main.get_prefix(message.guild.id)}info"):
            if not message.mentions:
              await message.reply(main.info(message.author.id,message.guild.id, message.author.name))
            elif main.check_dresseur_existe(message.mentions[0].id,message.guild.id):
              await message.reply(main.info(message.mentions[0].id,message.guild.id, message.mentions[0].name,message.author.id))
            else :
              await message.reply(f"Dresseur `{message.mentions[0].name}` introuvable.")
          ...
          newline='\n'
          await message.reply(f"<a:mokeball:958666482894643200>`CLASSEMENT DES DRESSEURS`<a:mokeball:958666482894643200>{newline*2}{newline.join([f'{numbers[i]} <@{classement[i][1]}> : {classement[i][2]}pts - {classement[i][3]} mots' for i in range(len(classement))])}{newline*2}`{stats[3]}` <@{message.author.id}> (Vous) : {stats[1]}pts - {stats[2]} mots")
          
        elif message.content == (f"{main.get_prefix(message.guild.id)}hauthautbasbasgauchedroitegauchedroiteBAstart"):
          await message.reply(main.cheatpoints(message.author.id,message.guild.id))
    if message.content:
